...
```{r}
spotify_songs |>
  summarise(dance_mean = mean(danceability))
  
spotify_songs |>
  summarise(dance_sd = sd(danceability))
...
This is a non-normal distribution that skews heavily to the right. The mean and standard deviation tell us that most songs in this dataset are considered "danceable," with a very small minority of songs falling below a 0.5 score. A story could be that in 2020, Spotify observed that most of the popular songs uploaded to its platform were very danceable, which may show where listener behavior was trending while the world was in the midst of pandemic lockdowns. 
