...
The variables that appear to have the strongest correlation are income/pct_bachelors_higher and turnout/pct_unemployed. The variables pct_bachelors_higher/median_age don't appear to have any correlation, either postive or negative. Income and population don't appear to have a linear relationship with turnout, while pct_unemployed does.
...
Looking at the multiple r-squared value, we can see that variables explain about 85 percent of the turnout scores. Three of the variables--population, pct_bachelors_higher and pct_unemployed--do not have statistically significant p-values, and therefore do not appear predictive. Overall, this is a useful model, as shown by the high multiple r-squared and adjusted r-squared values.
...
Model 3 was better than Model 2 and Model 1, but not significantly so. Model 3 included the three variables with statistically significant p-values as well as pct_unemployed, which, although it did not have a statistically significant p-value, it was closer than the other two insignificant variables.
...
# Model 3: Only Percent Variables
model3 <- lm(turnout ~  pct_bachelors_higher + 
                   pct_unemployed + pct_white, data = md_turnout)
summary(model3)

# Model 4: Model 2, but keeping Near-Significant Variables
...
If I were on a reporting team in an election year, I would use these results to steer the direction of my election forecasting and election coverage. These results show which demographic variables are most impactful in voter turnout, which will ensure that me and my team focus our reporting in the right directions. Otherwise, we could overemphasize something like voters' education levels, which our analysis showed us does not significantly impact voter's turnout.
