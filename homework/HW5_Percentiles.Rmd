...
ANSWERE HERE: 13 cities are above the 90th percentile. The IQR is 17. This new information doesn't change my original story idea, since that idea was based in percentage change comparisons across the state. While most other cities saw reductions in violent crime, Ocean City's spiked. IQR doesn't affect that much, since we're comparing percentage change, not raw numbers. 
```{r}
md_cities_percentiles <- md_cities_change |>
  mutate(percentiles10 = ntile(violent2020, 10)) 

md_cities_percentiles |>
  summarise(IQR(violent2020, na.rm = FALSE, type = 7))
