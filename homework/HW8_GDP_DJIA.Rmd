...
```{r}
cor(gdp_djia$GDP, gdp_djia$DJIA)
```
...
```{r}
ggplot(gdp_djia, aes(x = GDP, y = DJIA)) +
  geom_point() +
  labs(title = "GDP vs DJIA",
       x = "GDP",
       y = "DJIA")
```
...
The scatterplot shows a strong, postive correlation between DJIA and GDP, which is supported the correlation coefficient of 0.95. As GDP increases, the scatterplot shows that DJIA is likely to increase, and vice versa. 
